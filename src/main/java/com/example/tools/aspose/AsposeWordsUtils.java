...
public class AsposeWordsUtils {
    ...
    public static Document docReplaceWithAsposeWords(InputStream inputStream, Map<String, String> map) {
        Document doc = null;
        try {
    ...
    public static Document docReplaceWithAsposeWords(byte[] bytes, Map<String, String> map) {
        Document doc = null;
        try {
            doc = new Document(new ByteArrayInputStream(bytes));
            replace(doc, map);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static String docToHtml(Document document) {
        if (document == null) {
        ...
        // 反转义 HTML 标签
        //return StringEscapeUtils.unescapeHtml(new String(os.toByteArray(), StandardCharsets.UTF_8));
        return new String(os.toByteArray(), StandardCharsets.UTF_8);
    }

    public static String docToHtml(byte[] bytes) {
        if (bytes == null) {
            throw new NullPointerException("bytes must be not null");
        }
        return docToHtml(new ByteArrayInputStream(bytes));
    }

    public static String docToHtml(InputStream inputStream) {
        if (inputStream == null) {
            throw new NullPointerException("inputStream must be not null");
        }
        try {
            return docToHtml(new Document(inputStream));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return "";
    ...
    public static String docToHtml(File file) throws FileNotFoundException {
        if (file == null) {
            throw new NullPointerException("file must be not null");
        }
        return docToHtml(new FileInputStream(file));
    }

    public static File htmlToDoc(String html, String path, String name, int saveFormat) throws FileNotFoundException {
        return htmlToDoc(html, path + name, saveFormat);
    }

    public static File htmlToDoc(String html, String path, int saveFormat) throws FileNotFoundException {
    ...
    public static File htmlToDoc(byte[] bytes, String path, String name, int saveFormat) {
        return htmlToDoc(bytes, path + name, saveFormat);
    }


    public static File htmlToDoc(InputStream is, String path, int saveFormat) {
        File file = null;
        try {
            Document doc = new Document(is);
            file = htmlToDoc(doc, path, saveFormat);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return file;
    ...
    public static File htmlToDoc(byte[] bytes, String path, int saveFormat) {
        return htmlToDoc(new ByteArrayInputStream(bytes), path, saveFormat);
    }

    public static File htmlToDoc(Document document, String path, int saveFormat) throws FileNotFoundException {
        File file = new File(path);
        OutputStream os = new FileOutputStream(file);
        try {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        InputStream inputStream = new FileInputStream(new File("C:\\Users\\WangYang\\Desktop\\1.docx"));
        ...
        map.put("{{content}}", "<html>\n" +
                ...
                " </body>\n" +
                "</html>");
//        Document document = docReplaceWithAsposeWords(inputStream, map);
        String s = docToHtml(inputStream);
        System.out.println(s);
        htmlToDoc(s, "C:\\Users\\WangYang\\Desktop\\1.pdf", SaveFormat.PDF);
    }
